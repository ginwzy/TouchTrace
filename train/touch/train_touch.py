...
Sequences are truncated to max_steps and dynamically padded per batch.
...
from features import load_trajectory_sequences
...
def log_train_devices() -> None:
    import tensorflow as tf

    gpus = tf.config.list_physical_devices("GPU")
    print(f"TensorFlow {tf.__version__} GPU={gpus}")
    if not gpus:
        print("WARNING: no Metal GPU visible; training on CPU.")
...
def train(lite: bool = False, epochs: int | None = None, data: Path | None = None) -> Path:
...
def main(argv: list[str] | None = None) -> None:
    ...
    train(lite=args.lite, epochs=args.epochs, data=args.data)
